...
http_server::http_server(utility::string_t url) : m_listener(url) {
  ...
  std::shared_ptr<DealerTable> tbl =
      std::make_shared<DealerTable>(nextId, 8, 6);
...
void http_server::handle_get(http_request message) {
  ...
  if (paths.empty()) {
    message.reply(status_codes::OK,
                  TablesAsJSON(U("Available Tables"), s_tables));
    return;
  }
...
void http_server::handle_post(http_request message) {
  ...
  if (paths.empty()) {
    ...
    message.reply(status_codes::OK,
                  BJPutResponse(ST_PlaceBet, tbl->AsJSON()).AsJSON());
    return;
  }
  utility::string_t wtable_id = paths[0];
  const utility::string_t table_id = wtable_id;
  ...
  if (table->Players.size() < table->Capacity) {
    ...
    if (cntEntry != query.end() && !cntEntry->second.empty()) {
      ...
      message.reply(status_codes::OK,
                    BJPutResponse(ST_PlaceBet, table->AsJSON()).AsJSON());
    } else {
      message.reply(status_codes::Forbidden,
                    U("Player name is required in query"));
    }
  } else {
...
void http_server::handle_put(http_request message) {
  ...
  if (paths.empty() || queryItr == query.end()) {
    message.reply(status_codes::Forbidden,
                  U("TableId and request are required."));
  }
  utility::string_t wtable_id = paths[0];
