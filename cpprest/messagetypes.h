...
struct BJHand {
  ...
  BJHand()
      : state(HR_Empty), result(HR_None), bet(0.0), insurance(0),
        revealBoth(true) {}
...
struct BJTable {
  ...
  static BJTable FromJSON(const web::json::object &object) {
    ...
    for (auto iter = players.as_array().begin();
         iter != players.as_array().end(); ++iter, i++) {
...
struct BJPutResponse {
  ...
  BJPutResponse() {}
  BJPutResponse(BJStatus status, web::json::value data)
      : Status(status), Data(data) {}

  static BJPutResponse FromJSON(web::json::value object) {
    return BJPutResponse((BJStatus)(int)object[STATUS].as_double(),
                         object[DATA]);
  }
...
inline web::json::value TablesAsJSON(
