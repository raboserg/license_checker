...
void DealerTable::Deal() {
  ...
  for (size_t player = m_currentPlayer + 1; player < Players.size(); player++) {
    ...
    if (Players[player].Hand.state == HR_Active) {
      m_responses[player]->reply(
          status_codes::OK,
          BJPutResponse(ST_YourTurn, this->AsJSON()).AsJSON());
  ...
  for (size_t i = 1; i < Players.size(); i++) {
    if (i != m_currentPlayer) {
      if (m_responses[i]) {
        m_responses[i]->reply(
            status_codes::OK,
            BJPutResponse(ST_Refresh, this->AsJSON()).AsJSON());
        m_responses[i].reset();
      } else {
...
void DealerTable::DoubleDown(http_request message) {
  if (m_currentPlayer == 0 ||
      Players[m_currentPlayer].Hand.state != HR_Active) {
...
void DealerTable::Wait(http_request message) {
  ...
  auto query =
      http::uri::split_query(http::uri::decode(message.relative_uri().query()));
  auto itr = query.find(QUERY_NAME);
  if (itr == query.end()) {
    message.reply(status_codes::Forbidden,
                  U("name and amount are required in query"));
    return;
  } else
    name = itr->second;
  ...
  if (playerIdx > 0) {
    ...
    if (m_pendingrefresh[playerIdx] != ST_None) {
      message.reply(
          status_codes::OK,
          BJPutResponse(m_pendingrefresh[playerIdx], this->AsJSON()).AsJSON());
...
void DealerTable::Insure(http_request message) {
  ...
  if (playerIdx > 0) {
    ...
    if (dealer.cards.size() < 1 || dealer.revealBoth ||
        dealer.cards[0].value != CV_Ace) {
...
void DealerTable::PayUp(size_t idx) {
  ...
  if (m_responses[idx]) {
    m_responses[idx]->reply(
        status_codes::OK, BJPutResponse(ST_PlaceBet, this->AsJSON()).AsJSON());
...
void DealerTable::DealerDeal() {
  ...
  for (size_t i = 1; i < Players.size(); i++) {
    ...
    if (player.Hand.state == HR_BlackJack) {
      player.Hand.result =
          (dealersHand.state == HR_BlackJack) ? HR_Push : HR_PlayerBlackJack;
      continue;
    }
...
// TODO: Right now, players must make sure to get in when the dealer is taking
// bets.
//       Change the logic to anticipate that a new player may come to the table
//       at any point, and wait until the next round.
