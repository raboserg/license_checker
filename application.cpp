...
void posix_death_signal(int signum) {
  LOGGER::getInstance()->critical(TM("ABORTED"), __FILE__, __LINE__,
                                  __FUNCTION__);
...
int main(int argc, const char *argv[]) {
  ...
  PARSER::getInstance2nd(LIC_INI_FILE)->get_value(U("FILES.lic_file_name"));
  PARSER::getInstance2nd(LIC_INI_FILE)
      ->get_value(U("FILES.lic_file_name-----"));

  LOGGER::getInstance()->info(TM("fSDFWSFSFSFSAf"), __FILE__, __LINE__,
                              __FUNCTION__);
  ...
  if (license_process_path.empty()) {
    LOGGER::getInstance()->warning(TM("Not found license of path"), __FILE__,
    ...
    try {
      ...
      if (se.code().value() == error_create_lic) {
        ...
        /*std::unique_ptr<const wchar_t *> dfsdfd =
        license_checker::char_to_wchar(se.what()); utility::string_t
        sfdsfd(dfsdfd.release()); const wchar_t *fdsfd = dfsdfd.release();
