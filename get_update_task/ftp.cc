...
// FTP uses two TCP connections to transfer files : a control connection and a
// data connection connect a socket(control socket) to a ftp server on the port
// 21 receive on the socket a message from the ftp server(code : 220) send login
// to the ftp server using the command USER and wait for confirmation (331) send
// password using the command PASS and wait for confirmation that you are logged
// on the server (230) send file: use the passive mode: send command PASV receive
// answer with an IP address and a port (227), parse this message. connect a
// second socket(a data socket) with the given configuration use the command STOR
// on the control socket send data through the data socket, close data socket.
...
ftp_t::ftp_t(const char *host_name, const unsigned short server_port)
    : tcp_client_t(host_name, server_port), m_server_port(server_port),
      sock_ctrl(0), sock_data(0) {}
...
void ftp_t::login(const char *user_name, const char *pass) {
  ...
  // Note: there is no space between the user name and CRLF; example of request
  // is "USER me\r\n"
...
void ftp_t::get_file(const char *file_name, const char *path) {
  char buf_request[255];
  std::string str_server_ip;
  std::string str_rsp;
  ...
  if ("213" == str_code) {
    std::string str_size = str_rsp.substr(
        4, str_rsp.size() - 2 -
               4); // subtract end CRLF plus start of "213 ", 213 SP
  ...
  receive_all(sock_data, file_name, path);
...
void ftp_t::create_socket(int &sock, const char *server_ip,
...
// PASV request asks the server to accept a data connection on a new TCP port
// selected by the server PASV parameters are prohibited The server normally
// accepts PASV with code 227 Its response is a single line showing the IP
// address of the server and the TCP port number where the server is accepting
// connections RFC 959 failed to specify details of the response format.
// implementation examples
// 227 Entering Passive Mode (h1,h2,h3,h4,p1,p2).
// the TCP port number is p1*256+p2
///////////////////////////////////////////////////////////////////////////////////////

void ftp_t::parse_PASV_response(const std::string &str_rsp,
                                std::string &str_server_ip,
                                unsigned short &server_port) {
  unsigned int h[4];
  unsigned int p[2];
  ...
  sscanf(str_ip.c_str(), "%u,%u,%u,%u,%u,%u", &h[0], &h[1], &h[2], &h[3], &p[0],
         &p[1]);
...
// assumptions: connection was closed on the FTP server, allowing recv() to
// return 0
///////////////////////////////////////////////////////////////////////////////////////

void ftp_t::receive_all(int sock, const char *file_name, const char *path) {
  int recv_size; // size in bytes received or -1 on error
  const int flags = 0;
...
// send() 'size_buf' bytes
// TCP is a byte stream. There is no guarantee of a one to one relation between
// the number of items sent and the number of calls to send() or recv().
// send_all() accepts a buffer size as parameter and keeps looping on a send()
// call, (that returns the number of bytes sent) until all bytes are sent.
///////////////////////////////////////////////////////////////////////////////////////
